...
const UI_LABELS = {
    ...
    "alertJSONLoaded": {
        "en": "Character JSON loaded",
        "zh": "角色JSON加载成功",
        "ja": "キャラクターJSONを読み込みました"
    },
    "alertJSONInvalid": {
        "en": "Invalid JSON file",
        "zh": "JSON文件格式错误",
        "ja": "JSONファイルの形式が正しくありません"
    },
    "alertGenerateFirst": {
...
function setCheckboxGroup(className, values){
...
function applyCharacterData(data){
    ...
    setCheckboxGroup("nationality", data.nationality);
    setCheckboxGroup("ethnicity", data.ethnicity);
    setCheckboxGroup("personality", data.personality);

    setValue("customNationality", "");
    setValue("customEthnicity", "");
    setValue("customPersonality", "");
    setValue("customOccupation", "");

    setValue("occupation", Array.isArray(data.occupation) ? data.occupation[0] : data.occupation);

    setValue("height", data.body?.heightCm);
    setValue("weight", data.body?.weightKg);
    setValue("bodyType", data.body?.bodyType);
    setValue("breastShape", data.body?.breastShape);
    setValue("hipShape", data.body?.hipShape);
    setValue("bust", data.body?.bustCm);
    setValue("waist", data.body?.waistCm);
    setValue("hips", data.body?.hipsCm);

    setValue("hairLength", data.hair?.length);
    setValue("hairColor", data.hair?.color);
    setValue("hairTexture", data.hair?.texture);
    setValue("hairCondition", data.hair?.condition);
    setValue("bangsShape", data.hair?.bangs);
    setValue("hairStrands", data.hair?.strands);
    ...
    setValue("faceShape", data.appearance?.faceShape);
    setValue("neck", data.appearance?.neck);
    setValue("skinTexture", data.appearance?.skinTexture);
    setValue("skinMarks", data.appearance?.skinMarks);
    setValue("skinScar", data.appearance?.skinScar);
    setCheckboxGroup("piercing", data.appearance?.piercings);

    setValue("onesuit", data.clothing?.onesuit);
    setValue("top", data.clothing?.top);
    setValue("bottom", data.clothing?.bottom);
    setValue("underwear", data.clothing?.underwear);
    setValue("clothingMaterial", data.clothing?.material);
    setValue("clothingPattern", data.clothing?.pattern);
    setCheckboxGroup("clothingDetail", data.clothing?.details);
    setValue("hand", data.clothing?.hand);
    setValue("handShape", data.clothing?.handShape);
    setValue("leg", data.clothing?.leg);
    setValue("legType", data.clothing?.legType);
    setValue("stockingsType", data.clothing?.stockingsType);
    setValue("shoes", data.clothing?.shoes);
    setValue("posePreset", data.clothing?.posePreset);

    setValue("artStyle", data.styleAtmosphere?.artStyle);
    setValue("lighting", data.styleAtmosphere?.lighting);
    setValue("cameraShot", data.styleAtmosphere?.cameraShot);
    setValue("mood", data.styleAtmosphere?.mood);
